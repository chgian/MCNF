...
public class LP {
	...
	/**
	 * Set up a new Linear Programming Problem. The problem has the form<br>
	 * min c'x s.t. blo(1:n)<=x<=bup(1:n), blo(n+1:n+m)<=Ax<=bup(n+1:n+m)<br>
	 * the system matrix A is given as sparse matrix
	 * 
	 * @param g
	 *            double[n]: cost vector
	 * @param dels
	 *            double[n_nonzero]: nonzero elements of A
	 * @param mcol
	 *            int[n_nonzero]: column indices of nonzeros of A
	 * @param mrow
	 *            int[n_nonzero]: row indices of nonzeros of A
	 * @param blo
	 *            double[n+m]: lower bounds array: first variable bounds then
	 *            constraint bounds
	 * @param bup
	 *            double[n+m]: upper bounds array: first variable bounds then
	 *            constraint bounds
	 * @exception IllegalArgumentException
	 *                thrown if array dimensions do not match
	 */
	public LP(double[] g, int[] mcol, int[] mrow, double dels[], double[] blo,
			double[] bup) {
		int i;
		...
		if (m < 1) {
			System.err
					.println("LP: Vector of lower bounds has wrong dimension:");
			System.err.println("LP: dim blo[] = " + blo.length + ".");
			System.err.println("LP: Since n = dim g[] = " + g.length
					+ " this implies m = " + m);
			throw new IllegalArgumentException("Inconsistent Array Dimensions");
	 ...
	 *            double[n]: cost vector
	 * @param A
	 *            double[m][n]: System matrix of system (array of rows)
	 * @param blo
	 *            double[n+m]: lower bounds array: first variable bounds then
	 *            constraint bounds
	 * @param bup
	 ...
	 *            double[]: cost vector
	 * @param A
	 *            double[][]: System matrix (Array of rows)
	 * @param cl
	 *            double[]: lower bounds on variables
	 * @param cu
	 *            double[]: upper bounds on variables
	 * @param rl
	 *            double[]: lower bounds on constraints
	 * @param ru
	 *            double[]: upper bounds on constraints
	 * @param n
	 *            number of variables (only first n columns of A and entries of
	 *            g, cl, cu are used)
	 * @param m
	 *            number of constraints(only first m rows of A and entries of
	 *            rl, ru are used)
	 * @exception IllegalArgumentException
	...
	public LP(double[] g, double[][] A, double[] cl, double[] cu, double[] rl,
		...
		if (A[0].length < n || cl.length < n || cu.length < n || g.length < n) {
			throw new IllegalArgumentException("Some array column dimension <n");
		}
		if (A.length < m || rl.length < m || ru.length < m) {
			throw new IllegalArgumentException("Some array row dimensions <m");
		}

		if (n < 1 || m < 1) {
			throw new IllegalArgumentException("Either n or m is zero");
		}
	 ...
	 * 
	 * @param iprint
	 *            0=none, 1=status info, 10=EMSOL message dump (dubugging),
	 *            default=0
	 */
	...
	/**
	 * Print out information about the LP solution on the screen solve() needs
	 * to be called first
	 */
	...
	public void print() {
		...
		for (int i = 0; i < n; i++) {
			System.out.println(i + ": " + widthPrint(df.format(clo[i]), 8)
					+ " " + widthPrint(df.format(cup[i]), 8));
		}
		System.out.println("");

		System.out.println("Upper and Lower bounds on constraints (rows):");
		for (int i = 0; i < m; i++) {
			System.out.println(i + ": " + widthPrint(df.format(rlo[i]), 8)
					+ " " + widthPrint(df.format(rup[i]), 8));
		}
		System.out
				.println("----------------------------- End of LP Data ----------------------------");
	 ...
	 * 
	 * @return the multiplier vector: first n locations are multipliers on
	 *         variable bounds, next m are multipliers on constraints
	 */
	 ...
	 * 
	 * @return ifail: 0=OK, 1=infeasible LP, >1 other failure
	 */
	public int getIfail() {
		if (!solved)
			return 99;
	...
	/**
	 * returns basis information returns an int[n+m] containing information on
	 * optimal basis in EMSOL format
	 * 
	 ...
	 * 
	 * @param bas
	 *            int[n+m] containing basis information in EMSOL format
	 */
	...
	/**
	 * Test method. defines a small 1x2 LP solves it and prints the solution
	 * 
	 * min -x_1 -x_2 s.t. 0 <= 0.5*x_1 + 2*x_2 <= 1 0 <= -2*x_1 -0.5*x_2 <= 1 0
	 * <= x_1, x_2 <= 2
	 */
