...
public class Mcnf2 {
	...
	public static void main(String[] args) throws Exception {
		FileReader file = new FileReader("data\\smallnet.dat");
		...
		while (!converged) {
			...
			for (int i = 0; i < commoditiesItems; i++) 
			{
				...
				double sum = getSums(mp.get(getKeysByValue(mp, i + commoditiesItems)),commoditiesItems, matrixB);
				System.out.println("	Cost (using original edge costs) = "+ cost[i + counter - commoditiesItems]);
				double cstar = get_cstar(i, matrixB, commod, lambdas_star);
				System.out.println("	RED_COST: cstar = " + cstar
						+ " col_cost = " + cost[i + counter - commoditiesItems]+ "=> red_cost = " + (sum - cstar));
				if (!converged)
					System.out.println("	=> add column "+ (i + counter - commoditiesItems) + " for demand "+ i);
				else
