...
public class Graph {
	...
	public void addEdge(String vert1, String vert2, double cost) {
		...
		if (i1 == -1) {
			throw new IllegalArgumentException("Vertex " + vert1
					+ " not in graph");
		}

		if (i2 == -1) {
			throw new IllegalArgumentException("Vertex " + vert2
					+ " not in graph");
		}
		...
		edge1.end = i2;
		edge1.cost = cost;
		edge1.cost_initial = cost;	
		edge1.capacity = 0;				// not assigned by user
		edges.add(edge1);
	}
	
	/**
	 * NEW FUNCTION
	 * Add an edge to the graph. The same as above but user can add capacity
	 * 
	 * @param vert1
	 *            source vertex
	 * @param vert2
	 ...
	 *                thrown if one of the vertices does not exist
	 */
	public void addEdge(String vert1, String vert2, double cost, double capacity)
	{
		int i1, i2;
		...
		System.out.println("Add Edge: " + vert1 + " - " + vert2 + " cost=" + cost + " cp=" + capacity);
	...
	/**
	 * NEW FUNCTION
	 * returns the number of nodes in vertices
	 * @return
	 * 			the number of nodes in vertices
	 */
	...
	/**
	 * NEW FUNCTION
	 * returns the number of edges in list edges
	 * @return
	 * 			number of edges
	 */
	 ...
	 * 
	 * @param i
	 *            index of edge (edges are numbered in the order they were
	 *            registered (starting at 0)
	 * @param cost
	 *            new cost/length of edge
	 */
	public void newCost(int i, double cost) {
		...
		edge.cost = cost;
		System.out.println("New Cost: " + vertices.get(edge.start) + " - "
				+ vertices.get(edge.end) + " : " + cost);
	...
	/**
	 * finds all shortest paths through the graph from start to target vertex
	 * and returns its length (uses Dijkstra's method)
	 * 
	 * @param start
	 *            start vertex of required shortest path
	 * @param target
	 *            end vertex of shortest path
	...
	private void criticalPath() {
		...
		do {
			 ...
			 * +noSources[i]+" "+done[i]+" "+to[i]+" "+dist[i]); }
			 */
			// find !done node with no sources
			inext = -1;
			for (i = 0; i < nvert; i++) {
				if (!done[i] && noSources[i] == 0)
					inext = i;
			}
	 ...
	 * @param to
	 *            current vertex
	 * @return index of egde leading to this vertex (edges are indexed in the
	 *         order they were registered, starting at 0)
	 */
	...
	/**
	 * get distance to vertex on shortest path or distance from this vertex to
	 * final one on critical path
	 * 
	 * @param to
	 *            current vertex
	...
	/**
	 * returns all vertices on the shortest path to given vertex as an
	 * ArrayList.
	 * 
	 * @param to
	 *            current vertex
	...
	/**
	 * returns a boolean array indicating for each edge whether it is used on
	 * the last calculated shortest path. The return array is a boolean array of
	 * dimension (no of edges) An array entry is true if the corresponding edge
	 * is used. Edges are numbered in the order in which they were registered.
	 * 
	 * @param to
	 *            final vertex of path
	...
	public String getEdgeName(int i) {
		...
		String name = ((String) vertices.get(e.start)) + " - "
				+ ((String) vertices.get(e.end));
		return name;

	}

	/**
	 * This is just a test method for the Graph class. The main method
	 * constructs a simple Graph and finds the shortest path.
	 */
...
class Edge {
	...
	public int end;
	public double cost;
	public double cost_initial;		// NEW the initial cost, because during column generation algorithm, cost will change 
	public double capacity;			// NEW the capacity of the edge
