...
public class Mcnf 
{
	...
	public static int readDataFile(String filename)
	{
		...
		{			   			   
		   System.out.println("Error in reading file " + filename + " at line " + lineIndex + ".\nError returned is:"+ e.getMessage());	
		   return -4;
		}	
		return 0;
	 ...
	 * Solution found when reduced costs are positive and no new path is added
	 * @return
	...
	public static int solveCommodityFlow()
	{
		...
		while(true)
		{
			...
			iteration_number++;
			int old_numberOfPaths = numberOfPaths;		// by comparing this value with the new numberOfPaths of array A, we know if we converged.
			...
			for( int dem=0; dem<ND; dem++)
			{
				...
				System.out.println("*************** Commodity " + dem + ": " + dmc.fromNode + "-" + dmc.toNode + " " + dmc.demand);
				...
				try
				{
					...
					System.out.println("\tNew path " + graph.getPath() + " \n\tWe will check first its reduced cost");
					...
					// use old_numberOfPaths for not having problems if in the meanwhile new columns are added
					double c_star = 0;
					for( int col=0; col<old_numberOfPaths; col++)
					{
						if( A[NE+dem][col]==1)		// this column refers to commodity in dmc
						{
							double path_cost = costArray[col];		// we set the path_cost equal with the corresponding cost in costArray
							
							for( int edge = 0; edge<NE; edge++)		// for all edges in path add (lambda)*r*D
							{	
								if( jlp != null)	// jlp exists
									//path_cost = (graph.getEdge(edge).cost_initial + Math.abs(jlp.lam[old_numberOfPaths+edge]))*dmc.demand;
									path_cost += Math.abs(jlp.lam[old_numberOfPaths+edge])*A[edge][col];
							}
					...
					// we will find the cost only for the new path that is stored in usededges
					double ric = 0;
					for( int edge = 0; edge<NE; edge++)		// for all edges in path add (cost+lambda)*r*D
					{
						...
						{	
							if( jlp != null)	// jlp exists
								ric += (graph.getEdge(edge).cost_initial + Math.abs(jlp.lam[old_numberOfPaths+edge]))*dmc.demand;	
							else
								ric += graph.getEdge(edge).cost_initial*dmc.demand;	
						}
					}							
					reduced_cost = ric-c_star;
					String ss = String.format("\tFor new path: c*=%3.4f, column cost= %3.2f, reduced cost = %3.4f", c_star, ric, reduced_cost);
					...
					// continue by adding the new column
					if( reduced_cost<-0.1)	// add the new column only if the reduced cost is negative
					{							
						...
						numberOfPaths++;
						System.out.println("\tThe new path was added in array A: Number of paths=" + numberOfPaths);
					}
					else
						System.out.println("\tThe new path was not added in array A");
				}
				catch(Exception e)
				{
			...
			// if no new lines added in array A, then solution found
			// Check if solution found, print the solution and exit while loop.
			if( old_numberOfPaths == numberOfPaths)		// no new paths added to array A. Solution found
			{				
					System.out.println("\n**************************************Solution found!!*************************************");	
					...
					for( int demandrow=NE; demandrow<ND+NE; demandrow++ )
					{
						...
						for( int demandcolumn = ND; demandcolumn<numberOfPaths; demandcolumn++)
						{
							if( A[demandrow][demandcolumn]==1)		// we found a column that corresponds to demand, print the path
							{
								...
								{
									String sss = String.format("\t weight= %1.4f and path cost= %3.3f", jlp.x[demandcolumn], pathcost*amountOfDemand);//*jlp.x[demandcolumn]);
			...
			else	// solution not found yet
			{
				...
				String ss = String.format("\nNo Solution found yet: total cost value is %3.4f", objective_value_cost);
				System.out.println(ss);				
			}		
			/////////////////// We printed solution ////////////////////////////////////////////////////////			
			...
			costArray = new double [numberOfPaths];
			...
			// in LP solver are denoted with x. 
			...
			try
			{
				...
				objective_value_cost = jlp.getValue();			
